...
"""
        ...
        doc_temp = SimpleDocTemplate(temp_buffer, pagesize=letter, leftMargin=inch, rightMargin=inch,
                                    topMargin=1.5*inch, bottomMargin=inch)
        doc_temp.build(story)
        total_pages[0] = len(doc_temp.pageTemplates[0].frames) if hasattr(doc_temp, 'pageTemplates') and doc_temp.pageTemplates else 1

        # Si no se pudo contar, usar un valor estimado
        if total_pages[0] == 0:
            # Estimación basada en longitud del contenido
            content_length = sum(len(str(item)) for item in story)
            estimated_pages = max(1, content_length // 8000)  # Aproximadamente 8000 caracteres por página
            total_pages[0] = estimated_pages

        # Segunda pasada: generar PDF final
